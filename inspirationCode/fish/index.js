const RENDERER = {
    ...
    keyDown(e) {
        ...
        case 37:
        // left
            this.goldfishes[0].rotate('left');
            break;
        case 38:
        // alert('up');
            this.goldfishes[0].move = true;
            break;
        case 39:
        // alert('right');
            this.goldfishes[0].rotate('right');
            break;
        case 40:
        // alert('down');
            this.goldfishes[0].rotate('down');
            break;
        default:
    ...
    setParameters() {
        ...
        this.context = $('<canvas />').attr({ width: this.width, height: this.height }).appendTo(this.$container).get(0)
            .getContext('2d');
        this.goldfishes = [];
    ...
    createGoldfishes() {
        for (let i = 0, count = this.GOLDFISH_COUNT; i < count; i++) {
    ...
    propagateRipple(x, y) {
        ...
        const index = Math.round(x + this.width * y);
    ...
    processData() {
        ...
        const { data } = image;
        const { width } = this;
        const { currentHeights } = this;
        const { nextHeights } = this;
        ...
        for (var y = 1, lengthy = this.height - 1; y < lengthy; y++) {
            for (var x = 1, lengthx = width - 1; x < lengthx; x++) {
                const baseIndex = x + width * y;
                var index = baseIndex * 4;
                let height = (currentHeights[baseIndex - 1] + currentHeights[baseIndex + 1] + currentHeights[baseIndex - width] + currentHeights[baseIndex + width]) / 2 - nextHeights[baseIndex];
    ...
    render() {
        ...
        const gradient = this.context.createRadialGradient(this.x, this.y, 0, this.x, this.y, this.distance);
        gradient.addColorStop(0, 'hsl(200, 60%, 60%)');
        gradient.addColorStop(1, 'hsl(220, 60%, 40%)');
        this.context.fillStyle = gradient;
        this.context.fillRect(0, 0, this.width, this.height);

        for (let i = 0, count = this.goldfishes.length; i < count; i++) {
...
GOLDFISH.prototype = {
    ...
    init(toRandomize) {
        ...
        this.radius = 0;// this.getRandomValue(0, Math.PI * 2);
    ...
    render(context) {
        ...
        if (this.waitCount == 0) {
            const rate = Math.max(this.VELOCITY_LIMIT, Math.sin(this.theta));
            if (this.move) {
                this.x += this.vx * rate;
                this.y += this.vy * rate;
            }
